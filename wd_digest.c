...
handle_t wd_digest_alloc_sess(struct wd_digest_sess_setup *setup)
{
	...
	ret = wd_drv_alg_support(sess->alg_name, &wd_digest_setting.config);
	if (!ret) {
				 ...
				 struct wd_sched *sched)
{
	...
	wd_digest_setting.priv = STATUS_ENABLE;
...
int wd_digest_init(struct wd_ctx_config *config, struct wd_sched *sched)
{
	...
	ret = wd_ctx_drv_config("sm3", &wd_digest_setting.config);
	if (ret)
		goto out_uninit_nolock;

	ret = wd_alg_init_driver_nw(&wd_digest_setting.config);
	if (ret)
...
out_drv_deconfig:
	wd_ctx_drv_deconfig(&wd_digest_setting.config);
out_uninit_nolock:
...
void wd_digest_uninit(void)
{
	...
	wd_alg_uninit_driver_nw(&wd_digest_setting.config);
	wd_ctx_drv_deconfig(&wd_digest_setting.config);
		     ...
		     struct wd_ctx_params *ctx_params)
{
	...
	while (ret != 0) {
		...
		ret = wd_ctx_param_init_nw(&digest_ctx_params, ctx_params,
					alg, task_type, WD_DIGEST_TYPE, 1);
		if (ret) {
			if (ret == -WD_EAGAIN) {
				continue;
			}
		...
		wd_digest_init_attrs.task_type = task_type;
	...
	ret = wd_ctx_drv_config(alg, &wd_digest_setting.config);
	if (ret)
		goto out_uninit_nolock;
