...
static int init_ctx_config2(struct acc_option *options)
{
	...
	case AEAD_TYPE:
		if (options->mem_type == UADK_AUTO)
			...
			ret = wd_aead_init2_(alg_name, SCHED_POLICY_DEV, TASK_HW, &ctx_params);
		if (ret)
	...
	case DIGEST_TYPE:
		if (options->mem_type == UADK_AUTO) {
			...
			ret = wd_digest_init2_(alg_name, SCHED_POLICY_RR, options->task_type, &ctx_params);
		} else
			ret = wd_digest_init2_(alg_name, SCHED_POLICY_DEV, options->task_type, &ctx_params);
		if (ret)
