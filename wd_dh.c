...
static int wd_dh_common_init(struct wd_ctx_config *config, struct wd_sched *sched)
{
	...
	wd_dh_setting.priv = STATUS_ENABLE;
...
int wd_dh_init(struct wd_ctx_config *config, struct wd_sched *sched)
{
	...
	ret = wd_ctx_drv_config("dh", &wd_dh_setting.config);
	if (ret)
		goto out_uninit_nolock;

	ret = wd_alg_init_driver_nw(&wd_dh_setting.config);
	if (ret)
...
void wd_dh_uninit(void)
{
	...
	wd_alg_uninit_driver_nw(&wd_dh_setting.config);
	wd_ctx_drv_deconfig(&wd_dh_setting.config);
...
int wd_dh_init2_(char *alg, __u32 sched_type, int task_type, struct wd_ctx_params *ctx_params)
{
	...
	while (ret) {
		...
		ret = wd_ctx_param_init_nw(&dh_ctx_params, ctx_params,
					alg, task_type, WD_DH_TYPE, WD_DH_PHASE2);
		if (ret) {
			if (ret == -WD_EAGAIN)
				continue;
			goto out_driver;
	...
	ret = wd_ctx_drv_config(alg, &wd_dh_setting.config);
	if (ret)
		goto out_uninit_nolock;
